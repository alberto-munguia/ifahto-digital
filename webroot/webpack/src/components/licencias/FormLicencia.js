...
Vue.component('formLicencia', {
    props: {
        ...
        licencia   : { type: [Array, Object], required: true },
    ...
    methods: {
        ...mapActions({
            agregar: 'Licencia/agregar',
            editar : 'Licencia/editar',
        }),

        clearForm() {
            this.idsFiles = []
            this.$refs.dropzoneLicencias.removeAllFiles()
            this.$refs.formLicencia.reset()
        },

        handleSubmit(method) {
            this.$refs.formLicencia.validate().then(isValid => {
                if (isValid) {
                    this.licencia.multimedia_ids = this.idsFiles
                    
                    switch (method) {
                        case 'add':
                            this.agregar(this.licencia)
                            break;

                        case 'edit':
                            this.editar(this.licencia)
    ...
    template: `
        ...
        >
            ...
            >
                ...
                <b-col sm="8">
                    <b-form-input v-model="licencia.nombre" :class="classes"></b-form-input>
                    <div class="invalid-feedback">{{ errors[0] }}</div>
                </b-col>
            </validation-provider>
            ...
            <validation-provider
                ...
                <div class="col-sm-8">
                    <b-form-textarea v-model="licencia.descripcion" rows="4" :class="classes"></b-form-textarea>
                    <div class="invalid-feedback">{{ errors[0] }}</div>
                </div>
            </validation-provider>
            ...
            >
                ...
                <b-col sm="8">
                    <div class="input-group">
                        ...
                        <b-form-input type="number" step=".01" v-model="licencia.importe" :class="classes"></b-form-input>
                        <div class="invalid-feedback">{{ errors[0] }}</div>
                    </div>
                </b-col>
            </validation-provider>
            ...
            <validation-provider tag="div" class="form-group row" v-slot="{ errors, classes }">
                <label class="col-sm-4 control-label text-sm-right pt-2">Usuario</label>
                <b-col sm="8">
                    <v-select
                        placeholder="Selecciona..."
                        v-model="licencia.usuario_id"
